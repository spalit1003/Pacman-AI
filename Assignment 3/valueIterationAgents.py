...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def computeQValueFromValues(self, state, action):
        ...
        for i in self.mdp.getTransitionStatesAndProbs(state, action):
            qValAvg = qValAvg + i[1] * (self.mdp.getReward(state, action, i[0]) + self.discount * self.getValue(i[0]))
    ...
    def computeActionFromValues(self, state):
        ...
        for i in self.mdp.getPossibleActions(state):
            actionVal = self.computeQValueFromValues(state, i)
            if actionVal > maxVal:
            	actionBest = [i]
            	maxVal = actionVal 
        ...
        return random.choice(actionBest)
        util.raiseNotDefined()
