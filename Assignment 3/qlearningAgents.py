...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeActionFromQValues(self, state):
        ...
        if self.getLegalActions(state) == ():
            ...
            qState = util.Counter()
            for i in self.getLegalActions(state):
                qState.update({(state, i): self.getQValue(state, i)})

            actionBest = []
            maxVal = -100000000000
            for j in qState.items():
              if j[1] > maxVal:
                actionBest = [j[0][1]]
                maxVal = j[1]
              if j[1] == maxVal:
                actionBest.append(j[0][1])

            bestAction = random.choice(actionBest)
            return bestAction
        util.raiseNotDefined()
    ...
    def getAction(self, state):
        ...
        if legalActions == ():
            ...
            if util.flipCoin(self.epsilon):
                pickAction = random.choice(legalActions)
            else:
                pickAction = self.computeActionFromQValues(state)
        return pickAction
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        self.weights.incrementAll(list(featureVector.keys()),0)
    ...
    def update(self, state, action, nextState, reward):
        ...
        for i in self.weights.keys():
            self.weights[i] = self.weights[i] + self.alpha * featureVector[i] * difference
